      ...
      <AnimatePresence mode="wait">
        {!scanResult ? (
          <motion.div 
            ...
            className="card p-5 md:p-16 flex flex-col items-center justify-center text-center space-y-8 border-2 border-slate-900 shadow-[10px_10px_0_rgba(0,0,0,0.05)] overflow-hidden"
          >
            {/* Always rendered video element, hidden when not scanning */}
            <div className="relative w-full max-w-lg aspect-square md:aspect-video bg-black rounded-lg overflow-hidden border-4 border-slate-900" style={{ display: isScanning ? 'block' : 'none' }}>
              <video 
                ref={videoRef} 
              ...
              {/* Scanning Overlay inside video container */}
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <div className="w-48 h-48 md:w-64 md:h-64 border-2 border-indigo-500 rounded relative">
                  ...
                  <div className="absolute left-0 right-0 h-1 bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,1)] animate-scan-line"></div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-4 w-full">
              {!isScanning ? (
                <button onClick={startScanner} className="btn-primary px-16 py-5 text-xl w-full md:w-auto">
                  Launch Interface
                </button>
              ) : (
                <button onClick={stopScanner} className="btn-secondary px-16 py-5 text-xl w-full md:w-auto bg-white border-2 border-slate-900">
                  Abort Interface
                </button>
              )}
