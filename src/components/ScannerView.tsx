...
export default function ScannerView({ data, onUpdateData }: ScannerViewProps) {
    ...
    const startScanner = async () => {
        ...
        try {
            ...
            else msg = `Camera Error: ${err.message || 'Unknown error'}`;
            setError(msg);
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-10">
            ...
            {/* Hidden canvas for QR processing — always in DOM */}
            <canvas ref={canvasRef} className="hidden" />

            {/* Scanner container — video + overlay together in same box */}
            {isScanning && (
                <div className="relative bg-black rounded-3xl overflow-hidden aspect-[4/5] shadow-2xl border-[12px] border-slate-900">
                    <video
                        ref={videoRef}
                        playsInline
                        muted
                    ...
                    <div className="absolute bottom-12 left-0 right-0 px-12 flex justify-center">
                        <button onClick={stopScanner} className="bg-white text-slate-900 font-black uppercase tracking-widest px-10 py-4 rounded border-2 border-slate-900 shadow-[4px_4px_0_rgba(0,0,0,0.15)] hover:translate-y-0.5 transition-all">Abort Interface</button>
                    </div>
                </div>
            )}
            ...
            <AnimatePresence mode="wait">
                {!isScanning && !scanResult ? (
                    ...
                    >
                        ...
                        <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${cameraStatus === 'Camera active' ? 'text-emerald-500' :
                    ...
                    <motion.div
                        ...
                        className={`card overflow-hidden shadow-2xl border-[6px] ${scanResult?.status === 'granted' ? 'border-emerald-500' :
                                scanResult?.status === 'duplicate' ? 'border-amber-500' : 'border-rose-500'
                            }`}
                    >
                        <div className={`p-12 text-center text-white ${scanResult?.status === 'granted' ? 'bg-emerald-600' :
