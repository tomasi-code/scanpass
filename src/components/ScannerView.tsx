...
export default function ScannerView({ data, onUpdateData }: ScannerViewProps) {
  ...
  const validateTicket = (ticketId: string) => {
    ...
    if (!ticket) { processResult('invalid', undefined, "Ticket ID not found."); return; }
    if (ticket.status === 'Revoked') { processResult('invalid', ticket, "Ticket has been revoked."); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-10">
      ...
      {/* Video + canvas always in DOM so ref is always valid */}
      ...
      <AnimatePresence mode="wait">
        {!isScanning && !scanResult && (
          ...
          >
            ...
            <button onClick={startScanner} className="btn-primary px-16 py-5 text-xl">Launch Interface</button>
          ...
          >
            ...
            <div className="p-12 space-y-10 bg-white">
              ...
              <button onClick={resetScanner} className="btn-primary w-full py-6 text-xl flex items-center justify-center gap-4 group">
                <div className="p-2 bg-white/20 rounded group-hover:rotate-180 transition-transform duration-500"><RefreshCw size={24} /></div>
                Return to Buffer
              </button>
            </div>
          </motion.div>
